...
describe('Act I generator', () => {
  ...
  it('always connects spawn to the elevator', () => {
    for (let i = 0; i < SEEDS; i++) {
      for (const floor of [1, 2, 3, 4]) {
        ...
        const elevatorIdx = map.elevator.y * MAP_W + map.elevator.x;
        expect(
          reach.has(elevatorIdx),
          `seed prop-${i} floor ${floor}: elevator unreachable`,
        ).toBe(true);
      }
    }
  });

  it('places exactly one elevator', () => {
    for (let i = 0; i < 60; i++) {
      const map = generateFloor(streamFor(`elev-${i}`, 'mapgen', 2), 2);
      const count = map.tiles.filter((t) => t === Tile.Elevator).length;
      expect(count).toBe(1);
    }
  });
  ...
  it('is deterministic for a given seed and floor', () => {
    ...
    expect(a.elevator).toEqual(b.elevator);
  });
