...
function useItem(world: World, itemId: number, events: SimEvent[]): boolean {
  ...
  switch (item.kind) {
    ...
    case 'emp': {
      let hit = 0;
      for (const a of world.actors) {
        if (a.faction !== 'hostile' || a.hp <= 0 || !a.networked) continue;
...
function descend(world: World, events: SimEvent[]): boolean {
  const p = player(world);
  if (tileAt(world, p.x, p.y) !== Tile.Elevator) {
    events.push({ t: 'blocked' });
    return false;
  }
  if (!canDescend(world)) {
  ...
  if (world.floor >= FINAL_FLOOR) {
    world.won = true;
    world.salvage += 200;
    log(world, 'THE WARDEN IS DOWN. YOU TAKE THE SHAFT DOWN AND OUT.', 'good');
    events.push({ t: 'descend', floor: world.floor + 1 });
    return true;
  }

  decayOnDescent(world);
  world.player.power = Math.min(world.player.maxPower, world.player.power + 25);
  world.salvage += 40;
  buildFloor(world, world.floor + 1);
  events.push({ t: 'descend', floor: world.floor });
  log(world, `SUBLEVEL ${world.floor}. DEEPER.`, 'alert');
  return true;
}
...
function runScheduler(world: World, events: SimEvent[]): void {
  ...
  for (const actor of [...world.actors]) {
    ...
    while (actor.energy >= ACTION_COST && actor.hp > 0 && !world.dead) {
      ...
      if (p.hp <= 0) {
        world.dead = true;
        log(world, 'YOU BLEED OUT ON A FLOOR NOBODY WILL SWEEP.', 'bad');
      }
    }
  }
  saveStreams(world, streams);
