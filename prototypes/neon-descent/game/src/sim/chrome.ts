/**
 * Chrome and Instability (DESIGN.md §6.6).
 *
...
import { CHROME } from '../content/items.ts';
import type { Player, World } from './types.ts';

export type InstabilityTier = 'CLEAN' | 'STATIC' | 'DISSONANCE' | 'FRACTURE' | 'CYBERPSYCHOSIS';

export function tierFor(instability: number): InstabilityTier {
  if (instability >= 80) return 'CYBERPSYCHOSIS';
...
export const TIER_WARNING: Record<InstabilityTier, string> = {
  ...
  DISSONANCE: 'HACKS WILL SOMETIMES PICK THEIR OWN TARGETS. YOU HIT HARDER.',
  FRACTURE: 'THREAT COLOURS WILL SOMETIMES LIE TO YOU. YOU HIT MUCH HARDER.',
  CYBERPSYCHOSIS: 'YOU CANNOT CHOOSE TO LEAVE A FLOOR. CLEAR IT OR DIE ON IT.',
};
...
export function damageMultiplier(instability: number): number {
  switch (tierFor(instability)) {
    ...
    case 'CYBERPSYCHOSIS': return 1.50;
    default: return 1;
  }
}

/** At CYBERPSYCHOSIS the elevator stops being an option. */
export function canDescend(world: World): boolean {
  if (tierFor(world.player.instability) !== 'CYBERPSYCHOSIS') return true;
  return !world.actors.some((a) => a.faction === 'hostile' && a.hp > 0);
}

export function has(player: Player, kind: string): boolean {
  return Object.values(player.chrome).some((c) => c && c.kind === kind);
}

export function install(player: Player, kind: string): { ok: boolean; replaced?: string } {
  const chrome = CHROME[kind];
  if (!chrome) return { ok: false };
  const previous = player.chrome[chrome.slot];
  player.chrome[chrome.slot] = chrome;
  player.instability = Math.min(100, player.instability + chrome.instability);
...
export function sightRadius(world: World, base: number): number {
  let r = base;
  if (has(world.player, 'kiroshi')) r += 3;
...
export function speedBonus(player: Player): number {
  let s = 0;
  if (has(player, 'reflex')) s += 50;
  if (has(player, 'sandevistan')) s += 80;
  if (has(player, 'subdermal')) s -= 20;
  return s;
}

export function armorBonus(player: Player, ranged: boolean): number {
  let a = 0;
  if (has(player, 'subdermal')) a += 3;
  if (ranged && has(player, 'thermalweave')) a += 2;
  return a;
}

export function meleeBonus(player: Player): number {
  return has(player, 'gorilla') ? 4 : 0;
}

export function hackCost(player: Player, base: number): number {
  return has(player, 'bandwidth') ? Math.ceil(base * 0.6) : base;
}
...
export function passiveTracePerTurn(player: Player): number {
  return has(player, 'kiroshi') ? 0.2 : 0;
}
