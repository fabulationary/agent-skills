# NEON ELEVATION — Art Direction & UI Layout
...
### FLESH — everything grown: humans, and the things grown from us
...
### SODIUM — hazard light, warning stripes, Act I and II ambience
...
| **Hazard (environmental)** | SODIUM | Fire, electrified deck, steam, Act I and II lighting |
| **Unknown / unexplored** | VOID | Fog, remembered-but-not-visible tiles at 40% |
| **Grown** | FLESH | Humans *and* biosynths — see below |

**Never** use BLOOD for a non-threat or ACID for something you cannot breach. The palette
is a language.

**Biosynths are FLESH, never ACID, and this cost a redesign to get right.** The first Stray
sprite was acid-green because it read as "synthetic" — which broke the palette's own rule,
since ACID means *breachable* and a biosynth is the single thing in the game that is not.
They are drawn in the same ramp as human beings, which is both semantically correct and the
point: they were grown from us, they look it, and the player's tools do not care.

Biosynth identity therefore comes from **silhouette**, not hue: low, long, four-legged,
unlike anything else in the roster. That is exactly what ART-DIRECTION §0 is for.
...
**Shading:** light comes from above-left in Acts I and II — fixtures, bolted where a human put
them. Act III is lit from everywhere at once, because Nirvana has actual daylight, and that is
the most unsettling thing in the game. Flat fills with hand-placed
...
| Biosynth eyeshine | ACID | 1 tile | Slow, irregular — it is breathing |
| Muzzle flash | LIGHT | 2 tiles | 1 frame |
| The Firstborn | ACID | 6 tiles | Irregular |
...
- **Scanlines:** 1 px darkening every 3 logical px, 8% alpha. Subtle enough to be atmosphere,
  not a filter.
- **Chromatic aberration:** only during Instability glitches and Firstborn attacks. Never
...
- Kills leave a corpse tile that persists for the tier's lifetime — Act III's Regrowth makes
  this mechanically load-bearing
- Ascending fades to VOID over 400 ms, holds 200 ms, then fades in with the new tier's
