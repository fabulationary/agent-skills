# NEON ELEVATION — Technical Architecture
...
│   └── mapgen/              #   act1-yards.ts, act2-stacks.ts, act3-rings.ts, prefabs.ts, validate.ts
│
...
**Property/golden.** Mapgen over 10,000 seeds per act asserting: lift reachable from
...
2. **Turn engine + FOV.** Scheduler, shadowcasting, one dumb enemy that chases. The game is now
   a game.
3. **Mapgen Act I + ascent.** Freight-yard caverns, prefabs, connectivity validation, lift,
   4 tiers.
...
7. **The Warden.** Boss encounter, tier-4 loading-bay arena prefab.
