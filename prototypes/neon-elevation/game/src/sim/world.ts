...
export function buildFloor(world: World, floor: number): void {
  ...
  if (floor === FINAL_FLOOR) {
    ...
    const boss = makeActor(world, 'warden', map.lift.x, map.lift.y - 2);
    boss.aware = false;
...
export function createWorld(seed: string): World {
  ...
  buildFloor(world, 1);
  log(world, 'TIER 1. BASELINE. THE TETHER IS AWAKE.', 'alert');
  log(world, 'MOVE. STANDING STILL IS HOW CLIMBERS DIE.', 'info');
  return world;
}
