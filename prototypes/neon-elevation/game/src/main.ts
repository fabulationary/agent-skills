...
function onGesture(gesture: Gesture): void {
  ...
  if (tile.x === p.x && tile.y === p.y) {
    ...
    if (here === Tile.Lift) act({ kind: 'ascend' });
...
function inspect(x: number, y: number): void {
  ...
  if (enemy) {
    ...
    inspectText = t === Tile.Lift ? 'FREIGHT LIFT. TAP YOURSELF HERE TO GO UP.'
...
function drawGameOver(): void {
  ...
  drawText(g, `BEST       TIER 0${meta.bestFloor}`, panel.x + 4, y, P.e); y += 10;
...
// Expose a tiny debug surface for playtesting from the console. `spawn` and
// `give` exist because the fastest way to check a rule — does an EMP really do
// nothing to a Stray? — is to put one next to yourself and try it.
Object.assign(window as unknown as Record<string, unknown>, {
  ND: {
    ...
    spawn: (kind: string, dx: number, dy: number) => {
      const p = world.actors[0];
      const x = p.x + dx;
      const y = p.y + dy;
      if (!isWalkable(world, x, y)) world.tiles[y * world.w + x] = Tile.Floor;
      const actor = makeActor(world, kind, x, y);
      updateFov(world);
      dirty = true;
      return actor;
    },
    give: (kind: string) => {
      const template = ITEMS[kind];
      ...
      const item = {
        ...
        ...(kind === 'graftchip' ? { graftKind: 'slipstream' } : {}),
      };
