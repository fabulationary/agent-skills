...
export function drawScene(
  ...
  for (let vy = 0; vy < layout.viewRows; vy++) {
    for (let vx = 0; vx < layout.viewCols; vx++) {
      ...
      switch (world.tiles[i]) {
        ...
        case Tile.Lift: drawFloorTile(g, px, py, x, y); drawSprite(g, 'lift', px, py); break;
  ...
  for (let vy = 0; vy < layout.viewRows; vy++) {
    for (let vx = 0; vx < layout.viewCols; vx++) {
      ...
      if (world.tiles[i] === Tile.Lift) glow(g, cx, cy, 40, P.o, 0.26 * flicker);
