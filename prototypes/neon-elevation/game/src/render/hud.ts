...
export function drawStatus(
  ...
  drawText(g, `TIER 0${world.floor}`, 4, 26, SEM.interactive);
...
export function drawLog(
  ...
  // cut off at "TIER 1. UMBILICAL GAMMA IS AW" is worse than no message.
...
export function buildSlots(world: World): SlotDef[] {
  ...
  const priority = ['patchkit', 'baffle', 'scrub', 'powercell', 'emp', 'ampoule', 'graftchip'];
  const shown: string[] = [];
